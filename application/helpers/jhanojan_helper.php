...
{
	$CI =& get_instance();
	$CI->db->where("(username='$username' OR alt_username = '$username')");
	//$CI->db->or_where("alt_username",$username);
...
if (!function_exists('GetOptAgama')){
	...
	{
		$q = array('Islam', 'Kristem', 'Hindu', 'Budha', 'Katolik');
		$opt[''] = "- Agama -";
		foreach($q as $r)
		{
			$opt[$r] = $r;
...
function webmasterkid(){
	$CI=&get_instance();
	$group = $CI->session->userdata('chosen_kid');
	return $group;
}
function getkurs($id){
...
function _request($url,$param,$type='get')
    ...
    function gradeletter($n){
        ...
        $CI=&get_instance();
        $grade=$CI->db->query("SELECT letter FROM sv_letter_grade WHERE lowest<=$n AND highest>=$n")->row();
        return $grade->letter;
    }
?>
