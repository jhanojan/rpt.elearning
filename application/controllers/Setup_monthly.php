	...
	function get_flexigrid()
        {
            ...
            $this->db->select("sv_a.*,sv_b.title ta_,IF(sv_a.jenjang='all', sv_a.jenjang, sv_c.title) as jenjang_,IF(sv_a.tingkat='all', sv_a.tingkat, sv_d.title) as tingkat_,CONCAT('Rp ', FORMAT(sv_a.totalprice, 2)) as nominal_")->from("$this->utama sv_a");
	...
	function submit(){
		...
		foreach($GetColumns as $r)
		{
			...
			unset($data[$r['Field']."_temp"]);
                ...
                $data['totalprice']=str_replace(',','.',str_replace('.','',str_replace('Rp ','',$data['totalprice'])));
