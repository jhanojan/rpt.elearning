...
error_reporting(0);
	...
	function main($id)
	{
		...
		$data['js_grid']=$this->get_column($data['job_order']);
	...
	function get_column($id){
			...
			$colModel['jo'] = array('Job Order',110,TRUE,'left',2);
			$colModel['no_invoices'] = array('Invoice',200,TRUE,'left',2);
	...
	function get_flexigrid($id)
        {
			...
			//$this->db->where(array('job_order'=>$id));
	...
	function generateinvoice($jo,$tbl,$div,$mod){
		...
		$quo=str_replace('invoice_','',$mod);
			...
			addnumbering($mod);
			$numb=$data['number'];
			$lol=GetAll('sv_ai',array('job_order'=>'where/'.$jo))->result_array();

			foreach($lol as $dota){
					$detail['invoice']=$data['number'];
					$detail['desc']=$dota['b_desc'];
					$detail['idr']=$dota['b_amount'];
