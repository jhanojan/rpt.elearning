...
class Master_client extends CI_Controller {
		...
		var $utama ='master_client';
		var $title ='Master Client';
...
error_reporting(0);
	...
	function form($id=null){
		...
		$data['opt_sales']=GetOptAll('master_sales','-Sales-',array(),'name');
	...
	function submit(){
		...
		/* if(!$this->input->post('bank_acc_grp')){$data['bank_acc_grp']='N';}
		else{$data['bank_acc_grp']='Y';} 
		 */
		if($id != NULL && $id != '')
		{
		...
		{
				$data['code']=generatenumbering('master_client');
			...
			$ins=$this->db->insert('sv_'.$this->utama, $data);
			
			$id = $this->db->insert_id();
			if($ins){addnumbering('master_client');}
			$this->session->set_flashdata("message", 'Sukses ditambahkan');
		}
		if($this->input->post('redirect')!=NULL){
				$this->session->set_flashdata('clientbaru',$id);
				
				redirect('marketing_form_prospect/form/'.$this->input->post('formid'));}
