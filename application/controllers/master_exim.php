...
class Master_exim extends CI_Controller {
	
		var $utama ='master_exim';
		var $title ='Master exim';
...
error_reporting(0);
	...
	function get_column(){
            ...
            $colModel['currency'] = array('Currency',110,TRUE,'left',2);
            $colModel['udm'] = array('UDM',110,TRUE,'left',2);
	...
	function get_record(){
		...
		$valid_fields = array('id','code','name');
			...
			elseif($row->status=='s'){$status='Suspended';}*/
				...
				GetValue('code','master_metric',array('id'=>'where/'.$row->currency)),
				GetValue('code','master_metric',array('id'=>'where/'.$row->udm))
                        );
            }
	...
	function form($id=null){
		...
		$data['opt_metric']=GetOptAll('master_metric','-Metric-',array(),'name');
		$data['opt_service']=GetOptAll('ref_service','-Service-',array());
		$data['content'] = 'contents/'.$this->utama.'/edit';
		$data['opt_tax']=GetOptAll('ref_tax','-Tax-',array(),'name','code');
