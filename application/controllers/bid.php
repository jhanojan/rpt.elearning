...
error_reporting(0);
	...
	function get_column(){
            ...
            $colModel['email'] = array('Email',70,TRUE,'left',2);
            $colModel['phone'] = array('Phone',100,TRUE,'left',2);
            $colModel['budget'] = array('Budget',110,TRUE,'left',2);
            $colModel['story'] = array('Story',110,TRUE,'left',2);
            $colModel['cookie_id'] = array('VISITOR ID',110,TRUE,'left',2);
            $colModel['logged'] = array('Sent',110,TRUE,'left',2);
	...
	function get_record(){
			...
			elseif($row->status=='s'){$status='Suspended';}*/
                ...
                $row->id,
                $row->id,
                '<a href="'.base_url('bid').'/form/'.$row->id.'" class="list-group-ite">'.$row->name.'</a>',
				$row->email,
				$row->phone,
				$row->budget,
				$row->story,
