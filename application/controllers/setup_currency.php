...
class Setup_currency extends CI_Controller {
	
		var $utama ='setup_currency';
		var $title ='Setup Currency';
...
error_reporting(0);
	...
	function get_column(){
            ...
            $colModel['currency'] = array('currency',110,TRUE,'left',7);
            $colModel['curr_code'] = array('code',110,TRUE,'left',2);
            $colModel['kurs'] = array('kurs',110,TRUE,'left',2);
	...
	function get_record(){
		...
		$valid_fields = array('id','currency','curr_code','kurs','effective');
			...
			elseif($row->status=='s'){$status='Suspended';}*/
                ...
                $row->id,
				$row->id,
                GetValue('name','master_currency',array('id'=>'where/'.$row->currency)),
                $row->curr_code,
                $row->kurs,
                $row->effective,
                        );
	...
	function submit(){
		...
		$data['curr_code']=GetValue('code','master_currency',array('id'=>'where/'.$data['currency']));
