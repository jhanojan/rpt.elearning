...
class Setup_comission extends CI_Controller {
	
		var $utama ='setup_comission';
		var $title ='Setup Comission';
...
error_reporting(0);
	...
	function get_column(){
            ...
            $colModel['marketing'] = array('marketing',110,TRUE,'left',2);
		$colModel['effective'] = array('effective date',110,TRUE,'left',2);
		$colModel['import'] = array('import',110,TRUE,'left',2);
		$colModel['export'] = array('export',110,TRUE,'left',2);
		$colModel['trucking'] = array('trucking',110,TRUE,'left',2);
	...
	function get_record(){
		...
		$valid_fields = array('id','marketing','effective','import','export','trucking');
			...
			elseif($row->status=='s'){$status='Suspended';}*/
                ...
                $row->id,
				$row->id,
				GetValue('name','master_sales',array('id'=>'where/'.$row->marketing)),
				$row->effective,
				$row->import,
				$row->export,
				$row->trucking,
	...
	function form($id=null){
		...
		$data['opt_marketing']=GetOptAll('master_sales','-Sales-',array(),'name');
