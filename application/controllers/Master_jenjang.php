...
class Master_jenjang extends CI_Controller {
	
		var $utama ='master_jenjang';
		var $title ='Master Jenjang';
	...
	function listcol(){
            ...
            $colModel['title'] = array('Nama Jenjang',110,TRUE,'left',2);
            return $colModel;
	}
	...
	function get_record(){
			...
			elseif($row->status=='s'){$status='Suspended';}*/ 
				...
				foreach($colModel as $key=>$cm){
					if($key=='auth'){
                                        $record_items[$a][$b]="<a href='".base_url()."master_jenjang/auth/$row->id'>Menu</a>";
                                        
                                        }elseif($key=='prospek'){
                                        $record_items[$a][$b]="<a href='".base_url()."master_jenjang/prospek/$row->id'>Prospek Ref</a>";
	...
	function form($id=null){
		...
		//$data['opt']=GetOptAll('menu','-Parents-');
                $data['opt_leader']=GetOptAll('sv_master_jenjang','-Leader-',array(),'nama_lengkap');
