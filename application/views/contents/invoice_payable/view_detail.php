...
<style>
	...
	<input type="text" id="search" placeholder="Job Order" />
...
<script type="text/javascript">
	...
	$("#btnSearch").click(function(){ // lihat kode HTML diatas yg terdetak biru adalah ID button yg di jadikan paameter Click pada fungsi ini.
		...
		var p = [{name:'qtype', value:'jo'},{name:'query', value:s}]; // parameter yang dikirim
...
function gantistatus(val){
					...
					$.ajax({
					   type: "POST",
					   url: "<?php echo site_url($this->utama."_payable/changestatus");?>",
					   data: "v="+parseInt(val),
					   success: function(data){
					   	$('#flex1').flexReload();
						//alert(data);
