...
<div id="container" style=" height: auto; margin: 0 " class="col-md-12">
    ...
    <?php if ($laporan->num_rows() == 0) {
        ...
        <ul>
            <?php foreach ($laporan->result() as $lp) { ?>
                <li>
                    <?php echo $lp->title?> <span style='color:gray;font-size:8pt;'> Diupload :<?php echo tglindo($lp->created_on)?></span>
                    <a href="<?php echo base_url()?>files/laporan/<?php echo $lp->filez?>" download><i class="fa fa-download"></i></a>
                </li>
            <?php
            } ?>
        </ul>
