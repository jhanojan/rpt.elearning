...
function btn(com,grid)
{
	...
	if(com=='Account Information'){
		if($('.trSelected',grid).length==1){ 
			...
			//var items = $('.trSelected',grid);
			window.location = _base_url + 'ai/main/import_sea/' + $('td:nth-child('+ (1+$.inArray('idnya',abbr)) +')>div', '.trSelected',grid).text();
		} else {
			return false;
		} 
	}
	if(com=='Generate Invoice'){
		if($('.trSelected',grid).length==1){ 
			if(confirm('Generate Invoice  ' + $('.trSelected',grid).length + ' items?')){
				...
				$.ajax({
					type: "POST",
					url:  _base_url + 'invoice/generateinvoice/' + $('td:nth-child('+ (1+$.inArray('number',abbr)) +')>div', '.trSelected',grid).text()+'/import_sea_job/import/invoice_import',
					data: "items="+itemlist,
